...
<div id="formdiv">
	<div id="inputbox">
	 ...
	 <input type="text"  id="eid" name="e_id" class="box"  placeholder="john@nyit.edu" required>
	 </div><br>
	  <div id="inputbox">
		<label for="eid">Password</label>
	 <input type="password" class="box" name="password" placeholder="Password" id="password" required/>
	 </div><br>
 ...
 { $name = $_POST['name'];
   ...
   else
   {
	   ...
	   {   $query = mysqli_query($link,"SELECT * FROM stud_info WHERE e_id='".$eid."'");
		    ...
		    if($row_cnt==0)
		    {
		   $query1 = mysqli_query($link,"insert into stud_info (s_name,gender,e_id,dob,address,phone,s_cred,password) values('".$name."','".$gender."','".$eid."','".$dob."','".$address."','".$phone."','".$s_cred."','".$password."')");
		    	//echo $row_cnt;
